...
-> 척도의 종류와 그 필요성을 서술할 수 있다.(2, 3번째 학습 목표 합치기)
```
<!-- 새롭게 배운 내용을 자유롭게 정리해주세요.-->
...
![](https://github.com/bird-one-00/25-1_STATISTICS_Assignment/blob/main/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202025-02-08%20001222.png)
...
![](https://github.com/bird-one-00/25-1_STATISTICS_Assignment/blob/main/img/%EB%8B%A4%EC%9A%B4%EB%A1%9C%EB%93%9C.png)
...
```
답: PC 유저층에 집중하여 배너 광고를 올리는 것이 더 효과적이다.

풀이과정:
전체 광고 클릭 확률 P(클릭): (0.01*0.7) + (0.05*0.3) = 0.022
P(모바일 유저|클릭) = (0.01*0.7) / 0.022 ≈ 0.32
P(PC 유저|클릭) = (0.05*0.3) / 0.022 ≈ 0.68
...
```
1️⃣ X / p-value는 귀무가설이 참일 때, 표본과 같은 값이 나올 확률을 의미함
2️⃣ X / 귀무가설이 참일 때, 지금과 같은 데이터를 얻을 확률이 5%라는 의미
3️⃣ O
```
